...
async function processEmail(parsed) {
  ...
  if (!rows.length) {
    ...
    if (bodyEmails.length) addRows(await findEntitiesByEmails(bodyEmails), 'enviado');
  }
  ...
  for (const row of rows) {
    ...
    const correoRow = {
      sentido: row.sentido, fecha, asunto: subject, cuerpo: body.slice(0, 20000),
    ...
    imap.getBoxes('', (err, boxes) => {
      ...
      const search = (boxes, prefix) => {
        for (const [name, box] of Object.entries(boxes)) {
          ...
          if (box.attribs && box.attribs.map(a => a.toLowerCase()).includes('\\sent')) return fullName;
          if (box.children) { const f = search(box.children, fullName); if (f) return f; }
        }
        return null;
      };
      resolve(search(boxes, ''));
    });
  });
}
...
function runImapAccount(user, password) {
  return new Promise((resolve) => {
    const imap = new Imap({ user, password, host: IMAP_HOST, port: IMAP_PORT, tls: true,
    ...
    imap.once('ready', async () => {
      ...
      if (sentFolder) await pollBox(imap, sentFolder, [['SINCE', since]], false);
      else console.log('('+user+') No se encontró carpeta Sent.');
      imap.end();
      resolve();
    });
    imap.connect();
