...
export default function Seguros({ perfil }) {
  ...
  async function loadContactos(seguroId) {
    const { data } = await supabase.from('persona_contacto').select('id, nombre, apellidos, cargo, broker_cia, telefono, movil, email, email_2, observaciones').eq('seguro_id', seguroId).order('nombre')
    setContactos(data || [])
  }

  async function loadInmuebles(seguroId) {
    const { data } = await supabase.from('inmuebles').select('id, codigo, calle, piso, propietario_id, propietarios!inmuebles_propietario_id_fkey(id, nombre, apellidos), inmueble_propietarios(propietario_id, propietarios(id, nombre, apellidos))').eq('seguro_id', seguroId).order('codigo')
    setInmuebles(data || [])
  }
        ...
        <>
          ...
          <div className="detail-panel">
            ...
            <div className="panel-body">
              ...
              <div className="field-section">Inmuebles asegurados ({inmuebles.length})</div>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div className={`modal${readOnly ? ' modal-ro' : ''}`}>
            ...
            <div className="modal-body">
              ...
              {modal === 'edit' && tab === 'correos' && <Correos entidadTipo="seguro" entidadId={form.id} email={form.email} readOnly={readOnly} onCountChange={setCorreoCount} contactoIds={contactos.map(c => c.id)} />}
