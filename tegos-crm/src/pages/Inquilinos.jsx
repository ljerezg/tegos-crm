...
export default function Inquilinos({ perfil }) {
  ...
  async function selectRow(row) {
    setSelected(row)
    const [{ data: acc }, { data: cor }] = await Promise.all([
      supabase.from('accion_inquilino').select('*, responsable(nombre_responsable), tipo_contacto(tipo_contacto)').eq('inquilino_id', row.id).order('fecha', { ascending: false }),
      supabase.from('correo').select('id, asunto, fecha, sentido, remitente, destinatario').eq('inquilino_id', row.id).order('fecha', { ascending: false })
    ])
    setAcciones(acc || [])
    setCorreos(cor || [])
  }
  ...
  function sortedFiltered() {
    let data = rows.filter(r => {
      const matchSearch = ms([r.nombre, r.apellidos, `${r.nombre || ''} ${r.apellidos || ''}`, r.email, r.movil, r.dni_cif, r.inmuebles?.codigo], search)
        ...
        <>
          ...
          <div className="detail-panel">
            ...
            <div className="panel-body">
              ...
              <div className="field-section">Correos ({correos.length})</div>
              {correos.length === 0
                ? <div style={{ color: 'var(--text3)', fontSize: 13 }}>Sin correos</div>
                : (
                    ...
                    {correos.map(c => (
                      <div className="tl-item" key={c.id}>
                        ...
                        <div className="tl-content">
                          <div className="tl-text">{c.asunto || '—'}</div>
                          <div className="tl-meta">{fmtDate(c.fecha)} · {c.sentido === 'recibido' ? 'Recibido' : 'Enviado'} · {c.remitente || c.destinatario || ''}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                )}
              <div className="field-section">Acciones recientes ({acciones.length})</div>
